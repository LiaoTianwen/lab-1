...
def getUnit(name):
    ...
    while needNewChoice:
        ...
        if k.isdigit() and t.isdigit():
            ...
            print('Введите числовые параметра!')
    return unit, k, t

def graph(num, title, y, x):
    pyplot.subplot(2, 2, num)
    pyplot.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0.5, hspace=0.5)
    pyplot.grid(True)
...
if unitName == 'Идеальное дифференцирующее звено':
    print(color.Fore.RED + 'Выходная функция представляет собой идеальную импульсную функцию, которая не показана на графике!')
else:
    timeLine = []
...
if unitName == 'Безынерционное звено':
    ...
    for omega in omegaLine:
        ...
        phase.append(90-180*math.atan(t*omega)/numpy.pi)
